...
interface RedProfileScreenProps {
  onTabChange?: (tab: string) => void;
}

const RedProfileScreen = ({ onTabChange }: RedProfileScreenProps) => {
  return (
    <div className="mobile-container gradient-red min-h-screen flex flex-col">
      ...
      <div className="flex-1 px-4 pb-24 overflow-y-auto">
        ...
        <div className="flex items-center justify-between mt-4 mb-6">
          <h1 className="text-white text-2xl font-bold">@mentor_ali</h1>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-white font-semibold">Ali Khan</span>
            <VerifiedBadge variant="red" size="sm" />
          </div>
          <p className="text-white/70 text-sm">
            Mentor | Educator | Helping students achieve their dreams 🎓
          </p>
          <p className="text-white/70 text-sm">Based in Karachi 📍</p>
        </div>

        {/* Action Buttons */}
        <div className="flex gap-3 mb-6">
          <button className="flex-1 bg-red-accent text-white font-semibold py-2.5 rounded-lg">
            Edit Profile
          </button>
        ...
        <div className="grid grid-cols-3 gap-1">
          {[...Array(9)].map((_, i) => (
            <div 
              key={i} 
              className="aspect-square bg-gradient-to-br from-red-900/50 to-red-800/30 rounded-sm"
            />
          ))}
        </div>
      </div>
      
      <BottomTabBar theme="red" activeTab="Profile" onTabChange={onTabChange} />
    </div>
  );
};

export default RedProfileScreen;
